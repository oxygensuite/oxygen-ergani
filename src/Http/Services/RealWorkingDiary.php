...
class RealWorkingDiary extends Service
{
    /**
     * Retrieves the real employment diary entries for a branch on a given date.
     *
    ...
    public function handle(int $branchAa, DateTime|string $date): array
    {
        ...
        $entries = $data[$this->serviceCode()]['RealWorking'] ?? [];
        ...
        return array_map(
            fn(array $entry) => new RealWorkingResponse($entry),
            $entries,
        );
    }
